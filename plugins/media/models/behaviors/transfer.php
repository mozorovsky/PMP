...
require_once ROOT . DS . APP_DIR . '/plugins/media/libs/mm/src/Mime/Type.php';
	...
	function transfer(&$Model, $file) {
		...
		switch ($chain) {
			case 'file-upload-remote>>uploaded-file-local>>file-local':
				...
				if (!MediaValidation::pixels($temporary['pixels'], '640x480')) {
					$Media = Media_Process::factory(array('source' => $destination['file']));
					$Media->fitInside(640,480);
					$Media->store($destination['file'], array('overwrite' => TRUE));
				}
				break;
			case 'file-local>>>>file-local':
				$result = copy($source['file'], $destination['file']);
				if (!MediaValidation::pixels($source['pixels'], '640x480')) {
					$Media = Media_Process::factory(array('source' => $destination['file']));
					$Media->fitInside(640,480);
					$Media->store($destination['file'], array('overwrite' => TRUE));
				}				
