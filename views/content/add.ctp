...
<h2>Nová stránka</h2><br/>
<?php 	echo $this->Form->create('Content', array('url' =>  '/content/add/'.$this->data['Content']['name']));
	  	echo $this->Form->input('title', array( 'label' => 'Nadpis stránky: ')).'<br/>';
		echo $this->Form->input('name', array( 'label' => 'Název v odkazu: ')).'<div class="alert">Bez diakritiky a spec. znaků (pouze -_)</div>';
//		echo $this->Form->hidden('category');
		...
		echo $this->Form->input('area', array( 'label' => 'Oblast: ', 'after'	=> '<div class="clear"></div>', 'empty' => '', 'options' => $area)); 
